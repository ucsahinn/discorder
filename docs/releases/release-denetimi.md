...
Bu tablo, v2.0.15 public yüzeyi hazırlanırken çıkarılan non-destructive release temizliği planıdır. GitHub Release veya tag silme işlemi yapılmadı. Silme/cleanup için ayrıca `APPROVED - CLEAN RELEASES` onayı gerekir.
...
| --- | --- | --- | --- | --- |
| `v2.0.15` | Oluştur / latest yap | Tanılama loglarından çıkan Tarayıcı modu kapalı kapsamı, kötü internet/DNS retry davranışı ve kapanış hızı düzeltmeleri bu sürümde. | Authenticode yoksa kullanıcı Windows SmartScreen uyarısı görebilir. | GitHub Release workflow veya `gh release create v2.0.15 ... --latest`. |
| `v2.0.14` | Koru | Sertifikasız ama GitHub digest, SHA-256 ve manifest doğrulamalı otomatik update modu bu sürümde yayınlandı. | Latest olarak kalırsa v2.0.15 runtime düzeltmeleri kullanıcıya ulaşmaz. | v2.0.15 yayınlanınca latest güncellenir. |
| `v2.0.13` | Koru / release oluşturma | Tag pushlandı fakat workflow test çalıştırma yolu yüzünden paket üretiminde durdu; tag taşımak force/destructive olacağı için korunur. | Eski tag release'siz kalır; latest v2.0.15 ile düzeltilir. | Silme veya retag yok. |
| `v2.0.12` | Koru / release oluşturma | Tag daha önce pushlandı fakat release oluşmadı. Tag silmek destructive olduğundan korunur. | Eski tag release'siz kalır; latest v2.0.15 ile düzeltilir. | Silme yok. |
| `v2.0.11` | Koru | Remote ve local tag mevcut; önceki stabil yayın hattı. | Latest olarak kalırsa README v2.0.15 hazırlığıyla çelişebilir. | v2.0.15 yayınlanınca latest otomatik/manuel güncellenir. |
...
Bu public yüzey için anlamlı SemVer sürümü `v2.0.15` olarak belirlendi.
...
v2.0.15 GitHub Release yayınlanmadan önce şu koşullar gerekir:

- GitHub yayın yetkisi geçerli olmalı.
- `v2.0.15` tag'i doğru commit'e işaret etmeli.
- Release workflow veya local build `Discorder-2.0.15-win-x64.zip` üretmeli.
- `.sha256.txt` dosyası ZIP ile eşleşmeli.
- Secret scan temiz olmalı.
- Release body `docs/releases/v2.0.15.md` ile uyumlu olmalı.
